...
class Talos(Robot): # Robot继承了Body；Talos继承了Robot
    ...
    def __init__(self, simulator, urdf, model, q=None, verbose=True, useFixedBase=False):
        ...
        self._node = rclpy.create_node('t51_zmp_cmp_dcm_node')
...
def main(): 
    ...
    while rclpy.ok(): # Main loop
        ...
        if use_ankle_strategy:
            ...
            ankle_gains = (12.0, 2.0)  # 调整增益 (Kx, Kp)
    ...
    if DO_PLOT:
        ...
        output_path = os.path.join(image_dir, "t51_plot_EX2.png")
        plt.savefig(output_path)
        print(f"The image is saved in {output_path}")
        ...
        output_name = "no_control" if not use_ankle_strategy and not use_hip_strategy else \
                      ...
                      "both_control"
        dynamic_output_path = os.path.join(image_dir, f"t51_plot_{output_name}.png")
