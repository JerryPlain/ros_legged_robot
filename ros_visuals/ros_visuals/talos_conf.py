...
# Use direct path instead of rospack
current_dir = os.path.dirname(os.path.abspath(__file__))
# Fix the path calculation - go up to workspace root correctly
workspace_root = os.path.join(current_dir, '../../..')
talos_description = os.path.join(workspace_root, 'src/talos_description')

# Check if the path exists and print for debugging
print(f"Current dir: {current_dir}")
print(f"Workspace root: {workspace_root}")  
print(f"Talos description: {talos_description}")

urdf = os.path.join(talos_description, "robots/talos_reduced_no_hands.urdf")
path = os.path.join(talos_description, "meshes/../..")

# Check if URDF exists, if not try alternative locations
if not os.path.exists(urdf):
    print(f"URDF not found at: {urdf}")
    # Try alternative path
    alt_talos_description = os.path.join(current_dir, '../../../talos_description')
    alt_urdf = os.path.join(alt_talos_description, "robots/talos_reduced_no_hands.urdf")
    
    if os.path.exists(alt_urdf):
        talos_description = alt_talos_description
        urdf = alt_urdf
        path = os.path.join(talos_description, "meshes/../..")
        print(f"Using alternative path: {urdf}")
    else:
        print(f"Alternative URDF also not found at: {alt_urdf}")

print(f"Final URDF path: {urdf}")
print(f"URDF exists: {os.path.exists(urdf)}")
...
# step dimensions
step_size_x = 0.18              # step size in x direction
step_size_y = 0.18              # step size in y direction

# lip settings
g = 9.81                        # gravity magnitude
h = 0.75                        # walking height

# mpc settings
dt_mpc = 0.05                                            # sampling time interval for the mpc
